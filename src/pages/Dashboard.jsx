

export default function Dashboard(){
    return (
        <div className="centered-page-content">
            <h1>Welcome to the Rick and Morty Match Game</h1>
    
            <h2>About</h2>

            <p className="paragraphs-only">“Wubba lubba dub dub! You’ve stumbled into my 
                digital lab of chaos and questionable brilliance. 
                There’s science, sarcasm, and probably a few timelines 
                that shouldn’t exist. Stay sharp, stay hydrated, and 
                try not to explode — that’s my job.”</p>
            <img src="/rickmortydash.jpg" alt="dash pic" width="700px" />
            
        </div>
    
    )
}